    ...
    while n_points_removed < n_points_to_remove:
        ...
        if e0 == e1:
            ...
            for i in range(1, len(edges)):
                ...
                if (i != indice) and (edges[i][0] == e0 or edges[i][1] == e0 or edges[i][0] == e1 or edges[i][1] == e1) and (edges[i][0] != edges[i][1]):
                    costs[i], newpoints[i] = compute_cost(edges[i], Quadrics)
            

    new_vertices = np.zeros((vertices.shape[0] - n_points_to_remove, 3), dtype=np.float32)
    counter = 0
