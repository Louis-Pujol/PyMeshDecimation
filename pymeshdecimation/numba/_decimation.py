...
def _initialize_quadrics(points, triangles):
...
@nb.jit(nopython=True, fastmath=True, cache=True)
def _compute_boundary_quadrics(points, repeated_edges, triangles):
...
@nb.jit(nopython=True, fastmath=True, cache=True)
def _compute_cost(edge, quadrics, points):
    pt0, pt1 = edge
...
@nb.jit(nopython=True, fastmath=True, cache=True)
def _intialize_costs(edges, quadrics, points):
    n_edges = edges.shape[1]
    costs = np.zeros(n_edges)
    newpoints = np.zeros((n_edges, 3))

    for i in range(n_edges):
        costs[i], newpoints[i] = _compute_cost(edges[:, i], quadrics, points)
    ...
    while n_points_removed < n_points_to_remove:
        ...
        while i < noninf_limit:
            ...
            if (edges[i][0] == e0 or edges[i][1] == e0) and edges[i][0] != edges[i][1]:
                costs[i], newpoints[i] = _compute_cost(
...
def _replay_loop(points, quadrics, collapses_history):
    for i in range(len(collapses_history)):
        ...
        quadrics[e0] += quadrics[e1]
        newpoint = _compute_cost([e0, e1], quadrics, points)[1]
        points[e0] = newpoint
